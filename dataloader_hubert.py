...
class MyDataset(Dataset):
    def __init__(self, root_dir='/data/gaobowen/split_video_25fps_stable', mode='Rxxxxxxxxxx', only_vae = True, audio_feats_type="hubert", audio_window_add = 0, imgHW = 384):
    ...
    def __getitem__(self, index):
        ...
        while (abs(ref_idx - img_idx) < 100 or
        ...
        if self.audio_feats_type == "whisper":
            audio_feats = torch.tensor(audio_feats_npy[img_idx - self.audio_window_add:img_idx + img_seq_len + self.audio_window_add]).reshape(-1, 384) # (90 384) 
        elif self.audio_feats_type == "hubert":
...
if __name__ == "__main__":
    ...
    dataset = MyDataset(root_dir='/data/gaobowen/split_video_25fps_sdvae320', mode='Rx', only_vae = False, 
    ...
    for step, (input_vaes_tensor, audio_feats, real_vaes_tensor, real_imgs_tensor) in enumerate(dataloader):
        print(input_vaes_tensor.shape, audio_feats.shape, real_vaes_tensor.shape, real_imgs_tensor.shape)
